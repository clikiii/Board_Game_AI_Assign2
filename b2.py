...
def timing(func):
    def wrapper(*args, **kwargs):
        ...
        print(colors.CYAN, "avg time", sum(TIME_LIST)/len(TIME_LIST), colors.ENDC)
...
class MatrixNode:
    ...
    def get_mobility_value(self):
        ...
        if self.playing == Constant.SHEEP_MIN:
            ...
            for wp in w_poses:
                cnt = 0
                cnt += (wp[0]-1 <0 or (self.matrix[wp[0]-1][wp[1]] != Constant.EMPTY_CELL))
                cnt += (wp[0]+1 >4 or (self.matrix[wp[0]+1][wp[1]] != Constant.EMPTY_CELL))
                cnt += (wp[1]-1 <0 or (self.matrix[wp[0]][wp[1]-1] != Constant.EMPTY_CELL))
                cnt += (wp[1]+1 >4 or (self.matrix[wp[0]][wp[1]+1] != Constant.EMPTY_CELL))
                if cnt == 3: cnt += 3
                if cnt >= 4: cnt += 5
    ...
    def get_max(matrix_node: MatrixNode, alpha, beta, depth):
        ...
        for idx, child_mov in enumerate(matrix_node.get_child_moves()):
            ...
            if idx == 0: new_val = simple_ab_search(child, alpha, beta, depth-1)
                ...
                if new_val > alpha and new_val < beta: new_val = simple_ab_search(child, alpha, beta, depth-1)
...
def iter_deep_ab_search(matrix_node: MatrixNode) -> Move:
    ...
    for cur_depth in range(Constant.MIN_DEPTH, Constant.MAX_DEPTH+1, 2):
        ...
        for idx, child_mov in enumerate(matrix_node.get_child_moves()):
            child = get_next_node(matrix_node, child_mov)
            cur_val = simple_ab_search(child, Constant.NEG_INF, Constant.POS_INF, 3)
        ...
        print("best move inside", cur_depth, best_move)
        if (time.time() - start_t) >= Constant.SEARCH_TIME_LIMIT:
            # print("\nbreak", cur_depth)
            break
