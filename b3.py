    ...
    def get_mobility_value(self):
        ...
        if self.playing == Constant.SHEEP_MIN:
            ...
            for wp in w_poses:
                ...
                if wp[0]-2 >=0: cnt -= ((self.matrix[wp[0]-1][wp[1]] == Constant.EMPTY_CELL) and (self.matrix[wp[0]-2][wp[1]] == Constant.SHEEP_MIN))*3
                if wp[0]+2 <=4: cnt -= ((self.matrix[wp[0]+1][wp[1]] == Constant.EMPTY_CELL) and (self.matrix[wp[0]+2][wp[1]] == Constant.SHEEP_MIN))*3
                if wp[1]-2 >=0: cnt -= ((self.matrix[wp[0]][wp[1]-1] == Constant.EMPTY_CELL) and (self.matrix[wp[0]][wp[1]-2] == Constant.SHEEP_MIN))*3
                if wp[1]+2 <=4: cnt -= ((self.matrix[wp[0]][wp[1]+1] == Constant.EMPTY_CELL) and (self.matrix[wp[0]][wp[1]+2] == Constant.SHEEP_MIN))*3
...
def iter_deep_ab_search(matrix_node: MatrixNode) -> Move:
    ...
    for cur_depth in range(Constant.MIN_DEPTH, max_depth+1, 2):
        ...
        print("best move inside", cur_depth, best_move, value, matrix_node.playing)
    return best_move
